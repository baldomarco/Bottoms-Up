...
generate_survival_plot <- function(data, current_taxon, current_climate_period) {
  ...
  # Convert filtered_data to a data.frame to avoid potential tibble-related issues with survfit/ggsurvplot
  filtered_data_df <- as.data.frame(filtered_data)
  
  # Fit Kaplan-Meier survival model with both AgeClass and forest_cat
  fit <- survfit(Surv(time, recovered) ~ AgeClass + forest_cat, data = filtered_data_df)
  
  # Extract survival summary for optimal recovery time calculation
  surv_data_summary <- surv_summary(fit)
  
  # Calculate optimal recovery time for each group (AgeClass, forest_cat)
  # Define the recovery threshold (e.g., 80% recovered)
  recovery_threshold <- 0.80
  
  optimal_recovery_times <- surv_data_summary %>%
    group_by(AgeClass, forest_cat) %>%
    # Find the first time point where 'event' (proportion recovered) is >= recovery_threshold
    # If no such point, return NA
    summarise(
      Optimal_Recovery_Time_80pct = ifelse(
        any(event >= recovery_threshold), # Check if recovery_threshold is ever reached
        min(time[event >= recovery_threshold]), # Get the minimum time when it's reached
        NA_real_ # If not reached, set to NA
      ),
      .groups = 'drop'
    )
  
  # Calculate mean/median recovery time per AgeClass and forest_cat
  summary_stats <- filtered_data_df %>% # Use filtered_data_df here
    ...
    # Join with optimal_recovery_times
    left_join(optimal_recovery_times, by = c("AgeClass", "forest_cat")) %>%
    select(Taxon, ClimatePeriod, forest_cat, AgeClass, Mean_Recovery_Time, Median_Recovery_Time, N_Recovered, Optimal_Recovery_Time_80pct)
  
  
  # Generate survival plot using ggsurvplot
  p <- ggsurvplot(
    fit,
    data = filtered_data_df, # Use the data.frame version here
    ...
    legend.title = "Group", # Set legend title to reflect combined factor
    color = "strata" # Use strata to color by the combined factor (AgeClass + forest_cat)
  )

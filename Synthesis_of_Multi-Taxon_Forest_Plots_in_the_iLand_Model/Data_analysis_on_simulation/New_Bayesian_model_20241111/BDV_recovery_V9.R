...
# Load the data
BDV_predictors <- read_excel("C:/iLand/20230901_Bottoms_Up/Sources_bottoms_up/Jenik/final_table_imp/tables_for_stat/Bdv_predictors_table_BayesianMod_results_track/20_Bdv_predictors_table_BayesianMod_results_th_with_elevation_mng_DWC_GAMage_snags_tot_deadwood_tes - Copy.xlsx")
...
BDV_recovery_filtered <- BDV_recovery_filtered %>%
  filter(year >= 0 & year <= 350 & forest_cat %in% c("Native Broadleaves"  ,  "Non-Native Coniferous")) # "Old-Growth"

dataroot <- "C:/iLand/20230901_Bottoms_Up/20230914_plot_experiment/_project/output/Plausibility/Final_imp/"
saveRDS(BDV_recovery_filtered, file.path(dataroot, "BDV_recovery_filtered_V8.rds")) # VARIABLES NEEDED IN THE BDV STUDY - Bayesian_BDV_model_bryophytes_V2
...
pdf(paste0(dataroot, "BDV_recovery_all_V15.pdf"), height=9, width=16)
...
# 1 Plot G1: Bar plots for all taxa together
...
# 2 Plot G2: Time series for all taxa together
...
# 2 Plot G2: Time series for all taxa together, separated by forest category
G2_plot <- ggplot(G2_data, aes(x = year, y = mean_value, color = forest_cat, fill = forest_cat)) +
  geom_line(size = 1.2) +
  geom_ribbon(aes(ymin = min_value, ymax = max_value), alpha = 0.2) +
  labs(title = "Predictions Above Threshold Over Time (All Taxa)", 
...
plot_taxa_quartile <- function(df, taxa, quartile) {
  ...
  # (G4) Boxplot per et?? in cui si supera la soglia
...
plot_taxa_filtered <- function(df, taxa) {
  ...
  # (G4) Boxplot: Age Distribution Above Threshold
  G4 <- ggplot(df_filtered, aes(x = forest_cat, y = age, fill = forest_cat)) +
    geom_boxplot(outlier.shape = 1, outlier.color = "gray40", outlier.size = 1, outlier.alpha = 0.3) +
    labs(title = paste(taxa, "-", quartile, "- Age Distribution Above Threshold"),
         x = "Forest Category",
...
# write excel
writexl::write_xlsx(BDV_recovery_filtered, "C:/iLand/20230901_Bottoms_Up/20230914_plot_experiment/_project/output/BDV_recovery_data_all_V15_no_old_growth.xlsx")
...
# ANALYSIS
#------------------
BDV_recovery_data <- read_xlsx("C:/iLand/20230901_Bottoms_Up/20230914_plot_experiment/_project/output/BDV_recovery_data_all_V15_no_old_growth.xlsx")
BDV_recovery_data
...
# NEED TO OPEN A PDF WRITER AND GIVE IT THE ROOT, THE NAME, AND THE SIZE
dataroot <- "C:/iLand/20230901_Bottoms_Up/20230914_plot_experiment/_project/output/"
pdf(paste0(dataroot, "BDV_recovery_15_no_old_growth.pdf"), height=9, width=16)

...
colnames(BDV_predictors) <- gsub("^X", "", colnames(BDV_predictors))  # Remove any leading "X"

# Rename species columns (clean up numbers and special characters)
BDV_predictors_cleaned <- BDV_predictors %>%
  rename(
    Bryophytes = Epiphyticepixilicbryophytes0212,
    Lichens = Lichens0137,
    Macrofungi = Macrofungi2118,
    Beetles = Nonflyingbeetles0053,
    Moths = Moths0566
  )
...
stat_tests <- BDV_species_long %>%
  ...
  kruskal_test(Richness ~ managementtype)
...
pairwise_comparisons <- BDV_species_long %>%
  group_by(Species) %>%
  pairwise_wilcox_test(Richness ~ managementtype, p.adjust.method = "bonferroni")
...
G1 <- ggplot(BDV_species_long %>% filter(Species == "Bryophytes"), aes(x = managementtype, y = Richness)) +
  geom_boxplot(fill = "gray70", color = "black", outlier.shape = NA) +  # Boxplots
...
G2 <- ggplot(BDV_species_long %>% filter(Species == "Lichens"), aes(x = managementtype, y = Richness)) +
  geom_boxplot(fill = "gray70", color = "black", outlier.shape = NA) +  # Boxplots
  geom_jitter(aes(color = Site), width = 0.2, size = 2, alpha = 0.7) +  # Dots
  stat_pvalue_manual(pairwise_comparisons %>%
...
G3 <- ggplot(BDV_species_long %>% filter(Species == "Macrofungi"), aes(x = managementtype, y = Richness)) +
  geom_boxplot(fill = "gray70", color = "black", outlier.shape = NA) +  # Boxplots
  geom_jitter(aes(color = Site), width = 0.2, size = 2, alpha = 0.7) +  # Dots
  stat_pvalue_manual(pairwise_comparisons %>%
...
G4 <- ggplot(BDV_species_long %>% filter(Species == "Beetles"), aes(x = managementtype, y = Richness)) +
  geom_boxplot(fill = "gray70", color = "black", outlier.shape = NA) +  # Boxplots
  geom_jitter(aes(color = Site), width = 0.2, size = 2, alpha = 0.7) +  # Dots
  stat_pvalue_manual(pairwise_comparisons %>%
...
G5 <- ggplot(BDV_species_long %>% filter(Species == "Moths"), aes(x = managementtype, y = Richness)) +
  geom_boxplot(fill = "gray70", color = "black", outlier.shape = NA) +  # Boxplots
  geom_jitter(aes(color = Site), width = 0.2, size = 2, alpha = 0.7) +  # Dots
  stat_pvalue_manual(pairwise_comparisons %>%
...
G1 <- ggplot(filtered_data, aes(x = managementtype, y = Richness)) +
...
# Add one single p-value annotation (above entire plot)
G2 <- ggplot(filtered_data, aes(x = managementtype, y = Richness)) +
  geom_boxplot(fill = "gray70", color = "black", outlier.shape = NA) +  
  geom_jitter(aes(color = Site), width = 0.2, size = 2, alpha = 0.7) +  
  annotate("text", x = 1.5, y = max(filtered_data$Richness, na.rm = TRUE) + 10, 
           label = paste("KW p-value:", filtered_pvalues$p.signif), size = 5) +  
...
# Add one single p-value annotation (above entire plot)
G3 <- ggplot(filtered_data, aes(x = managementtype, y = Richness)) +
  geom_boxplot(fill = "gray70", color = "black", outlier.shape = NA) +  
  geom_jitter(aes(color = Site), width = 0.2, size = 2, alpha = 0.7) +  
  annotate("text", x = 1.5, y = max(filtered_data$Richness, na.rm = TRUE) + 10, 
           label = paste("KW p-value:", filtered_pvalues$p.signif), size = 5) +  
...
# Add one single p-value annotation (above entire plot)
G4 <- ggplot(filtered_data, aes(x = managementtype, y = Richness)) +
  geom_boxplot(fill = "gray70", color = "black", outlier.shape = NA) +  
  geom_jitter(aes(color = Site), width = 0.2, size = 2, alpha = 0.7) +  
  annotate("text", x = 1.5, y = max(filtered_data$Richness, na.rm = TRUE) + 10, 
           label = paste("KW p-value:", filtered_pvalues$p.signif), size = 5) +  
...
# Add one single p-value annotation (above entire plot)
G5 <- ggplot(filtered_data, aes(x = managementtype, y = Richness)) +
  geom_boxplot(fill = "gray70", color = "black", outlier.shape = NA) +  
  geom_jitter(aes(color = Site), width = 0.2, size = 2, alpha = 0.7) +  
  annotate("text", x = 1.5, y = max(filtered_data$Richness, na.rm = TRUE) + 10, 
           label = paste("KW p-value:", filtered_pvalues$p.signif), size = 5) +  
...
pdf(paste0(dataroot, "boxplot_with_stats.pdf"), height=8, width=12)
dev.off()

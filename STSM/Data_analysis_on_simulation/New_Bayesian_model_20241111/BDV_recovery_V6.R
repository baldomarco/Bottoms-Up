...
pdf(paste0(dataroot, "BDV_recovery_all_V9.pdf"), height=9, width=16)
...
plot_taxa <- function(df, taxa) {
  ...
  G1 <- df %>%
    ...
    labs(title = paste(taxa, "- Normalized Probability of Exceeding Threshold"),
         x = "Forest Category",
         y = "Normalized Probability of Exceeding Threshold") +
  ...
  G2 <- ggplot(summary_df, aes(x = year, y = mean_value, color = forest_cat, fill = forest_cat)) +
    ...
    labs(title = paste(taxa, "- Predictions Above Median Over Time"),
         x = "Year",
         y = "% of Predictions Above Median") +
    theme_minimal()

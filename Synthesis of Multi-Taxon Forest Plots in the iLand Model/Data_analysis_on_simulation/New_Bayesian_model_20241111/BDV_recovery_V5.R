...
#BDV_recovery_filtered <- BDV_recovery %>% filter(year >= 0 & year <= 250)

BDV_recovery_filtered <- BDV_recovery %>%
  filter(year >= 0 & year <= 250 & forest_cat %in% c("Native Broadleaves"  ,  "Non-Native Coniferous"))
...
plot_taxa_quartile <- function(df, taxa, quartile) {
  ...
  # 1️⃣ (G1) Bar plot: Numero di anni in cui la % supera 0 per Forest Category
  G1 <- df %>%
    filter(!!sym(col_name) > 0) %>%
    group_by(forest_cat) %>%
...
# write excel
writexl::write_xlsx(BDV_recovery_filtered, "C:/iLand/2023/20230901_Bottoms_Up/20230914_plot_experiment/_project/output/BDV_recovery_data_all_V7_no_old_growth.xlsx")
...
#------------------
BDV_recovery_data <- read_xlsx("C:/iLand/2023/20230901_Bottoms_Up/20230914_plot_experiment/_project/output/BDV_recovery_data_all_V7_no_old_growth.xlsx")
BDV_recovery_data

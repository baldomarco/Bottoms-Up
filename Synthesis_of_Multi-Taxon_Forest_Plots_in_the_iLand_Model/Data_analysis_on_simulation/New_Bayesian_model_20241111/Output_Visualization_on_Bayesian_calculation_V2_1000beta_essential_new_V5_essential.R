...
B3 <- ggplot(reduced_data, aes(x = year, y = PRED_RICH_MACROFUNGI)) +
  ...
  geom_line(data = Bayesian_BDV_model_V3_multi, 
            aes(x = year, y = MACROFUNGI_PRED_RICH_50, 
                linetype = "Median Function"),
            color = "darkred", size = 1, inherit.aes = FALSE) +
  
  geom_hline(yintercept = 192, color = "red", 
  ...
  stat_summary(data = BDV_predictors,
               aes(x = 400, y = Macrofungi),
               fun = median,
               geom = "point",
               color = "red",
...
MF1 <- ggplot(Bayesian_BDV_model_V3_multi, aes(x = year)) +
  ...
  geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta1, color = "Beta 1 age"), size = 0.9) +
  geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta2, color = "Beta 2 deadwood"), size = 0.9) +
  geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta3, color = "Beta 3 ba_broadl"), size = 0.9) +
  geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta4, color = "Beta 4 tree_10_40"), size = 0.9) +
  ...
  scale_x_continuous(
    breaks = seq(
      min(Bayesian_BDV_model_V3_multi$year),
      max(Bayesian_BDV_model_V3_multi$year),
      by = 40
    )
  ) +
  ...
  theme(
    ...
    # remove facet strip box, keep text
    strip.background = element_blank(),
...
B3 <- ggplot(reduced_data, aes(x = year, y = PRED_RICH_MACROFUNGI)) +
  ...
  scale_color_viridis(option = "H", name = "Density") +
  geom_line(
    data = Bayesian_BDV_model_V3_multi,
    aes(x = year, y = MACROFUNGI_PRED_RICH_50, linetype = "Median Function"),
    color = "darkred",
    linewidth = 0.8,
    inherit.aes = FALSE
  ) +
  geom_hline(yintercept = 192, color = "red", linetype = "dashed", linewidth = 0.4) +
  # Boxplot posizionato a fine asse (es. anno 380) per non sovrapporsi ai dati
  geom_boxplot(
    data = BDV_predictors,
    aes(x = 380, y = Macrofungi),
    width = 20,
    notch = TRUE,
    fill = NA,
    color = "black",
  ...
  scale_linetype_manual(values = c("Median Function" = "solid"), name = NULL) +
  labs(x = NULL, y = "Predicted sp. richness") +
  scale_y_continuous(limits = c(0, NA), breaks = pretty_breaks(n = 6)) +
  # Forza l'asse X a 400 anni
...
# --- PLOT 2: BETA VALUES ---
MF1 <- ggplot(Bayesian_BDV_model_V3_multi, aes(x = year)) +
  geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta1, color = "Beta 1 age"), linewidth = 0.7) +
  geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta2, color = "Beta 2 deadwood"), linewidth = 0.7) +
  geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta3, color = "Beta 3 ba_broadl"), linewidth = 0.7) +
  geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta4, color = "Beta 4 tree_10_40"), linewidth = 0.7) +
  labs(x = "Year", y = "Beta value", color = NULL) +
  scale_color_brewer(palette = "Dark2") +
  # Forza l'asse X a 400 anni
  scale_x_continuous(limits = c(0, 400), breaks = seq(0, 400, by = 100)) +
  facet_wrap(~run, ncol = 4) +
...
# --- COMBINAZIONE CON LETTERE (TAGS) ---
# tag_levels = 'a' assegna le lettere a, b, c, d... in ordine di aggiunta dei plot
Final_plot <- B3 / MF1 +
  plot_layout(heights = c(1.5, 1), guides = "collect") +
...
# Label names
stand_labs <- c(
  "1" = "Unmanaged beech stand",
  "2" = "Managed E. beech stand",
  "3" = "Unmanaged N. spruce stand",
  "4" = "Managed N. spruce stand"
)

#----------------------------------
# Plot 1 : BDV prediction - Density
B3 <- ggplot(reduced_data,
             aes(x = year, y = PRED_RICH_MACROFUNGI)) +
...
annotate("rect",
  ...
  # ---------------- Density ----------------
geom_pointdensity(alpha = 0.35) +                       # to change the brightess
...
geom_boxplot(
  ...
  labs(
    x = NULL,
    y = "Predicted wood-inhabiting Fungi sp. rich."
  ) +
  ...
  facet_wrap(
    ~run,
    ncol = 4,
    labeller = labeller(run = stand_labs)
  ) +
...
#-----------------------------------
# Plot 2 = beta x predictors

MF1 <- ggplot(Bayesian_BDV_model_V3_multi,
              aes(x = year)) +
...
annotate("rect",
  ...
  # ---------------- Betas ------------------
geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta1,
              color = "Age"),
          linewidth = 0.7) +
  
  geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta2,
                color = "DeadC"),
            linewidth = 0.7) +
  
  geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta3,
                color = "Ba_Broad"),
            linewidth = 0.7) +
  
  geom_line(aes(y = MACROFUNGI_PRED_RICH_50_beta4,
                color = "Tree_10_40"),
            linewidth = 0.7) +
  
  labs(
    x = "Number of years",
    y = "Beta coef. x predictor",
    color = "Predictors"
  ) +
